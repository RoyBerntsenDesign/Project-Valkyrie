...
void setup() {
  ...
  WiFi.setHostname(DRYBOX_NAME); //define hostname
  ...
  setupHX711();  // read eeprom and setup hx711 lib
...
void fillPage(AsyncWebServerRequest * request, String page, String content){
  ...
  AwsTemplateProcessor atp = 
    [refreshStr](const String& var){
      ...
      if(var == "CALIBRATION_SLOPE") return isCalibrated()? String(calSlope) : "Uncalibrated";
...
String buildPostData(){
  ...
  if(isCalibrated()){
    //int iNetWeight = round(getCalibratedScaleNetWeight());
    //str+=String(iNetWeight)+"g\"";
  ...
  if(isCalibrated()){
    //int iNetWeight = round(getCalibratedScaleWeight());
    //str+=String(iNetWeight)+"g\"";
